...
async function loadMonthData(month){
  ...
  state.entries = s1.docs.map(d=>({id:d.id, __month: month, ...d.data()}));
...
async function loadEntriesForCalc(centerMonth, backMonths=3, forwardMonths=1){
  // Load a small window of months so card payment schedule works even if you input past/future dates.
  // UI tables still show state.entries (selected month only). This is for calculations only.
  const months = [];
  for(let d=-backMonths; d<=forwardMonths; d++){
    months.push(addMonthsKey(centerMonth, d));
  }

  const all = [];
  // Always include currently loaded month entries (may include auto-posted fixed costs)
  (state.entries||[]).forEach(e=> all.push({ ...e, __month: e.__month || centerMonth }));

  for(const m of months){
    if(m===centerMonth) continue;
    try{
      const ref = collection(db, "months", m, "entries");
      const qx = query(ref, orderBy("occurredAt","asc"));
      const snap = await getDocs(qx);
      snap.docs.forEach(d=> all.push({ id:d.id, __month: m, ...d.data() }));
    }catch(e){
      // month collection may not exist yet; ignore
      console.warn("[entriesCalc] load skipped:", m, e?.message||e);
    }
  }
  state.entriesCalc = all;
}
    ...
    for(const fc of fixed){
      ...
      while(nextMs>=monthStart && nextMs<=monthEnd && safety<24){
        ...
        if(!already){
          ...
          state.entries.push({ id: "__tmp__"+Math.random().toString(36).slice(2), __month: month, ...payload });
...
function buildCardPaymentSchedule(){
  ...
  const entriesSrc = (Array.isArray(state.entriesCalc) && state.entriesCalc.length) ? state.entriesCalc : (state.entries||[]);
  const entries = entriesSrc.filter(e=>e.type==="expense" && e.paymentMethod==="クレカ").map(e=>{
...
$("#m_save").addEventListener("click", async ()=>{
    ...
    const targetMonth = monthKey(new Date(occurredAt));

    if(mode==="add"){
      payload.createdAt = Date.now();
      payload.createdBy = state.user.uid;
      await addDoc(collection(db, "months", targetMonth, "entries"), payload);
    }else{
      const fromMonth = (entry && entry.__month) ? entry.__month : state.month;
      if(fromMonth === targetMonth){
        await updateDoc(doc(db, "months", fromMonth, "entries", entry.id), payload);
      }else{
        // Move entry to the correct month collection (date-based)
        const keepCreatedAt = entry?.createdAt || Date.now();
        const keepCreatedBy = entry?.createdBy || state.user.uid;
        await deleteDoc(doc(db, "months", fromMonth, "entries", entry.id)).catch(()=>{});
        await addDoc(collection(db, "months", targetMonth, "entries"), { ...payload, createdAt: keepCreatedAt, createdBy: keepCreatedBy });
      }
    }
    hideModal();
    await reloadAll();
