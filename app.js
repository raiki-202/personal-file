...
async function loadMonthData(month){
  ...
  // people_persons / people_health（家族：基本情報＋健康情報）
  let persons = [];
  try{
    const sp = await getDocs(query(collection(db, "people_persons"), orderBy("name","asc")));
    persons = sp.docs.map(d=>({id:d.id, ...d.data()}));
  }catch(e){
    // backward compatibility: if collection not created yet
    persons = [];
  }

  // fallback to old family collection if people_persons is empty
  if(persons.length===0){
    const s5 = await getDocs(query(collection(db, "family"), orderBy("name","asc")));
    persons = s5.docs.map(d=>({id:d.id, ...d.data()})).map(p=>({
      ...p,
      // normalize keys to the new schema
      birth_date: p.birth_date || p.birthDate || "",
      name_kana: p.name_kana || "",
      is_living_with: (typeof p.is_living_with==="boolean") ? p.is_living_with : true,
  ...
  // normalize & keep legacy-friendly alias (state.family is used in other screens)
  state.people_persons = persons.map(p=>({
    ...p,
  ...
  state.family = state.people_persons;

  // health: docId = person_id（推奨）。person_id フィールドがある場合も吸収。
  state.people_health = [];
  state.peopleHealthByPersonId = {};
  try{
    const sh = await getDocs(collection(db, "people_health"));
    state.people_health = sh.docs.map(d=>({id:d.id, ...d.data()}));
    state.people_health.forEach(h=>{
      const pid = h.person_id || h.personId || h.id;
      if(pid) state.peopleHealthByPersonId[pid] = h;
    });
  }catch(e){
    state.people_health = [];
    state.peopleHealthByPersonId = {};
...
function renderFamily(){
  const list = (state.people_persons || state.family || []);
  const showInactive = !!state._showInactiveFamily;
  const rows = list
    .filter(x=> showInactive ? true : (x.active!==false))
    .map(x=>{
      const bd = x.birth_date || x.birthDate || "";
      const rel = x.relation ? escapeHtml(x.relation) : "-";
      const kana = x.name_kana ? escapeHtml(x.name_kana) : "";
      const living = (typeof x.is_living_with==="boolean") ? (x.is_living_with ? "同居" : "別居") : "-";
      const phone = x.phone_number ? escapeHtml(x.phone_number) : "-";
      const email = x.email ? escapeHtml(x.email) : "-";
      const notes = (x.notes || x.memo) ? escapeHtml(x.notes || x.memo) : "";
      const inactive = (x.active===false);
      return `
        <tr class="${inactive?'dim':''}">
          <td>
            <div style="font-weight:800;">${escapeHtml(x.name||"")}${kana?` <span class="muted" style="font-weight:600;">(${kana})</span>`:""}${inactive?` <span class="pill">無効</span>`:""}</div>
            <div class="small muted">${living}${(x.gender||"")?` / ${escapeHtml(x.gender)}`:""}</div>
          </td>
          <td>${rel}</td>
          <td>${bd ? escapeHtml(bd) : "-"}</td>
          <td>${phone}</td>
          <td>${email}</td>
          <td class="muted">${notes}</td>
          <td class="right">
            <button class="btn mini secondary" data-health-person="${x.id}">健康</button>
    ...
    <div class="card">
      ...
      <div class="tableWrap">
        <table class="table">
          ...
          <tbody>
            ${rows || `<tr><td colspan="7" class="muted">まだありません。</td></tr>`}
          </tbody>
        </table>
      </div>

      <div class="muted small" style="margin-top:10px;">
        ・誕生日などは「定期イベント（90日以内）」の候補にも自動で拾います。<br/>
        ・健康情報は person ごとに1件（people_health: docId=person_id 推奨）で管理します。
      </div>
    </div>
  `;
}
...
function wireViewEvents(){
  ...
  $$("[data-health-person]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const id = btn.dataset.healthPerson;
      openHealthModal(id, state.family.find(x=>x.id===id));
    });
  });
  $$("[data-del-family]").forEach(btn=>{
    btn.addEventListener("click", async ()=>{
      ...
      try{ await deleteDoc(doc(db, "people_health", id)); }catch(e){}
      await reloadAll();
    });
  });
...
function openFamilyModal(mode, item=null){
  ...
  const relVal = (v.relation||"").trim();

  const html = `
    <div class="grid2">
      <div>
        <label class="label">名前</label>
        <input id="p_name" class="input" value="${escapeHtml(v.name||"")}" placeholder="例：萌奈" />
      </div>
      <div>
        <label class="label">ふりがな</label>
        <input id="p_kana" class="input" value="${escapeHtml(v.name_kana||"")}" placeholder="例：もな" />
      </div>
    </div>
    ...
    <div class="grid2" style="margin-top:10px;">
      <div>
        ...
        <select id="p_relation" class="input">
          ...
          ${relationOpts.map(o=>`<option value="${o.v}" ${o.v===relVal?"selected":""}>${o.l}</option>`).join("")}
        </select>
      </div>
      <div>
        <label class="label">誕生日</label>
        <input id="p_birth" class="input" type="date" value="${escapeHtml(v.birth_date||v.birthDate||"")}" />
      </div>
    </div>
    ...
    <div class="grid2" style="margin-top:10px;">
      <div>
        ...
        <input id="p_gender" class="input" value="${escapeHtml(v.gender||"")}" placeholder="例：男 / 女 / その他" />
      </div>
      <div>
        <label class="label">同居</label>
        <div class="row center gap8" style="height:44px;">
          <input id="p_living" type="checkbox" ${(typeof v.is_living_with==="boolean" ? v.is_living_with : true) ? "checked" : ""} />
          <span class="muted">同居している</span>
        </div>
      </div>
    </div>
    ...
    <div class="grid2" style="margin-top:10px;">
      <div>
        ...
        <input id="p_phone" class="input" value="${escapeHtml(v.phone_number||"")}" placeholder="任意" />
      </div>
      <div>
        <label class="label">Email</label>
        <input id="p_email" class="input" value="${escapeHtml(v.email||"")}" placeholder="任意" />
      </div>
    </div>
    ...
    <div class="grid2" style="margin-top:10px;">
      <div>
        ...
        <div class="row center gap8" style="height:44px;">
          <input id="p_active" type="checkbox" ${v.active===false?"":"checked"} />
    ...
    <div style="margin-top:10px;">
      ...
      <textarea id="p_notes" class="input" rows="3" placeholder="自由メモ">${escapeHtml(v.notes||v.memo||"")}</textarea>
    </div>

    <div class="row gap8 end" style="margin-top:14px;">
      ${mode==="edit" ? `<button class="btn secondary" id="btnOpenHealth">健康</button>` : ``}
  ...
  if(mode==="edit"){
    const btnOpenHealth = $("#btnOpenHealth");
    if(btnOpenHealth){
      btnOpenHealth.addEventListener("click", ()=> openHealthModal(v.id || item?.id, v));
    }
  }
  ...
  $("#btnSaveFamily").addEventListener("click", async ()=>{
    ...
    const payload = {
      ...
      is_living_with: $("#p_living").checked,
    ...
    if(mode==="add"){
      payload.createdAt = Date.now();
      const d = await addDoc(collection(db, "people_persons"), payload);
      // health doc placeholder (optional)
      await setDoc(doc(db, "people_health", d.id), { person_id: d.id, updatedAt: Date.now(), createdAt: Date.now() }, { merge: true });
    }else{
      await updateDoc(doc(db, "people_persons", v.id), payload);
    }
    closeModal();
    await reloadAll();
  });
  ...
  if(delBtn){
    delBtn.addEventListener("click", async ()=>{
      if(!confirm("削除しますか？（健康情報も一緒に消えます）")) return;
      await deleteDoc(doc(db, "people_persons", v.id));
      // best-effort delete health
      try{ await deleteDoc(doc(db, "people_health", v.id)); }catch(e){}
      closeModal();
      await reloadAll();
    });
  }
...
function openHealthModal(personId, person=null){
  if(!personId){ alert("person_id がありません"); return; }
  if(state.role==="viewer"){ alert("viewer は編集できません"); return; }

  const h = (state.peopleHealthByPersonId && state.peopleHealthByPersonId[personId]) ? state.peopleHealthByPersonId[personId] : {};
  const p = person || state.family.find(x=>x.id===personId) || {};

  const html = `
    <div class="small muted" style="margin-bottom:10px;">対象：<b>${escapeHtml(p.name||"")}</b></div>

    <div class="grid2">
      <div>
        <label class="label">血液型</label>
        <input id="h_blood" class="input" value="${escapeHtml(h.blood_type||"")}" placeholder="例：A" />
      </div>
      <div>
        <label class="label">身長（cm）</label>
        <input id="h_height" class="input" inputmode="decimal" value="${escapeHtml(String(h.height??""))}" placeholder="任意" />
      </div>
    </div>
    ...
    <div class="grid2" style="margin-top:10px;">
      <div>
        ...
        <input id="h_weight" class="input" inputmode="decimal" value="${escapeHtml(String(h.weight??""))}" placeholder="任意" />
      </div>
      <div>
        <label class="label">最終健診日</label>
        <input id="h_last" class="input" type="date" value="${escapeHtml(h.last_checkup||"")}" />
      </div>
    </div>

    <div style="margin-top:10px;">
      <label class="label">アレルギー</label>
      <input id="h_allergies" class="input" value="${escapeHtml(h.allergies||"")}" placeholder="任意" />
    </div>

    <div style="margin-top:10px;">
      <label class="label">持病</label>
      <input id="h_chronic" class="input" value="${escapeHtml(h.chronic_diseases||"")}" placeholder="任意" />
    </div>

    <div style="margin-top:10px;">
      <label class="label">常用薬</label>
      <input id="h_meds" class="input" value="${escapeHtml(h.regular_medicine||"")}" placeholder="任意" />
    </div>

    <div style="margin-top:10px;">
      <label class="label">かかりつけ病院</label>
      <input id="h_hospital" class="input" value="${escapeHtml(h.hospital_name||"")}" placeholder="任意" />
    </div>

    <div style="margin-top:10px;">
      <label class="label">通院歴・手術歴</label>
      <textarea id="h_history" class="input" rows="3" placeholder="任意">${escapeHtml(h.medical_history||"")}</textarea>
    </div>

    <div style="margin-top:10px;">
      <label class="label">定期通院予定</label>
      <textarea id="h_regular" class="input" rows="2" placeholder="例：月1回（第2土曜）">${escapeHtml(h.regular_visit_plan||h.定期通院予定||"")}</textarea>
    </div>

    <div style="margin-top:10px;">
      <label class="label">ワクチン履歴</label>
      <textarea id="h_vaccine" class="input" rows="3" placeholder="任意">${escapeHtml(h.vaccination_history||"")}</textarea>
    </div>

    <div style="margin-top:10px;">
      <label class="label">健康メモ</label>
      <textarea id="h_notes" class="input" rows="3" placeholder="任意">${escapeHtml(h.notes||"")}</textarea>
    </div>

    <div class="row gap8 end" style="margin-top:14px;">
      <button class="btn" id="btnSaveHealth">保存</button>
    </div>
  `;
  ...
  $("#btnSaveHealth").addEventListener("click", async ()=>{
    ...
    const payload = {
      ...
      height: toNum($("#h_height").value),
      weight: toNum($("#h_weight").value),
      allergies: $("#h_allergies").value.trim(),
      chronic_diseases: $("#h_chronic").value.trim(),
      regular_medicine: $("#h_meds").value.trim(),
      hospital_name: $("#h_hospital").value.trim(),
      last_checkup: $("#h_last").value || "",
      medical_history: $("#h_history").value.trim(),
      regular_visit_plan: $("#h_regular").value.trim(),
      vaccination_history: $("#h_vaccine").value.trim(),
      notes: $("#h_notes").value.trim(),
      updatedAt: Date.now()
    };
    // docId = personId
    await setDoc(doc(db, "people_health", personId), payload, { merge:true });
    closeModal();
    await reloadAll();
  });
}
