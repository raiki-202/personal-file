...
function getAllAccountsFromMaster(){
  ...
  // Prepaid cards as accounts (editable like normal accounts)
  // id: pp_<prepaidId>, name: "<cardName>（プリペイド）"
  for(const p of (state.prepaidCards||[])){
    if(!p || !p.id) continue;
    const id = prepaidAccountIdForCardId(p.id);
    if(list.some(a=>a.id===id)) continue;
    list.push({ id, name: `${p.cardName||p.id}（プリペイド）`, type:"asset", system:false, active:true });
  }
...
function prepaidAccountIdForCardId(prepaidId){
  return `pp_${prepaidId}`;
}
...
function entryAccountLabel(e){
  ...
  if(e.type==="charge"){
    const srcLabel = (e.paymentMethod==="クレカ" && e.creditCardId) ? creditCardName(e.creditCardId) : accountName(e.fromAccountId||"-");
    const dstLabel = accountName(e.toAccountId||"-");
...
function accountDeltasFromEntries(){
  ...
  const pickDefaultBank = ()=>{
    const banks = (state.master?.banks||[]).filter(x=>x.active!==false);
    return banks[0]?.id || (state.master?.otherAccounts||[]).filter(x=>x.active!==false)[0]?.id || null;
  };
  const defaultBankId = state.master?.defaultBankAccountId || pickDefaultBank();
  ...
  for(const e of (state.entries||[])){
    ...
    if(e.type==="charge"){
      // Charge: subtract from payment source, add to prepaid destination
      let from = e.fromAccountId || null;
      let cardId = e.creditCardId || null;

      if(e.paymentMethod==="クレカ"){
        if(cardId) from = payableAccountIdForCardId(cardId);
      }else{
        if(!from){
          if(e.paymentMethod==="現金") from = cashId;
          else from = defaultBankId;
        }
      }

      const to = e.toAccountId; // pp_<prepaidId>
      if(from){ m.set(from, (m.get(from)||0) - amt); }
      if(to){ m.set(to, (m.get(to)||0) + amt); }
      continue;
    }
    ...
    <div class="card">
      ...
      <div class="tableWrap">
        <table class="table">
          ...
          <tbody>
            ${list.length===0 ? `<tr><td colspan="3" class="small">まだありません。</td></tr>` : list.map(p=>{
              const accId = prepaidAccountIdForCardId(p.id);
              const mb = mergedBalances();
              const bal = mb.find(x=>x.id===accId)?.balance ?? Number(p.balance||0);
              return `
                <tr>
                  <td>${escapeHtml(p.cardName||"")}</td>
                  <td class="right">¥${yen(bal)}</td>
                  <td class="right">
                    <button class="btn secondary" data-edit-prepaid="${p.id}">編集</button>
                    <button class="btn danger" data-del-prepaid="${p.id}">削除</button>
                  </td>
                </tr>
              `;
            }).join("")}
      ...
      <div class="sep"></div>
      <div class="small">※残高は「口座管理」の月末残高入力にも反映されます（推定残高計算の対象）。</div>
    </div>
  `;
}

function openPrepaidModal(mode, item=null){
  if(state.role==="viewer"){ alert("viewer は編集できません"); return; }
  showModal(mode==="add" ? "プリペイドカードを追加" : "プリペイドカードを編集", `
    <div class="formGrid">
      <div>
        <div class="small">カード名</div>
        <input id="pp_name" class="input" value="${escapeHtml(item?.cardName||"")}" />
      </div>
      <div>
        <div class="small">残高（初期値）</div>
        <input id="pp_balance" class="input" type="number" inputmode="numeric" value="${Number(item?.balance||0)}" />
      </div>
    </div>
  ...
  $("#pp_save").addEventListener("click", async ()=>{
    ...
    const balance = Number($("#pp_balance").value||0);
    if(!cardName){ alert("カード名を入力してください"); return; }

    const payload = { cardName, balance, updatedAt: Date.now() };
    if(mode==="add"){
      payload.createdAt = Date.now();
      payload.createdBy = state.user.uid;
      await addDoc(collection(db, "prepaidCards"), payload);
    }else{
      await updateDoc(doc(db, "prepaidCards", item.id), payload);
    }
    hideModal();
    await reloadAll();
  }, { once:true });
...
function wireViewEvents(){
  ...
  if(btnAddEntry){
    btnAddEntry.addEventListener("click", ()=>{
      const activeBtn = document.querySelector('[data-moneytab].tab.active');
      const t = activeBtn ? activeBtn.dataset.moneytab : (state._moneyTab||"income");
      openEntryModal("add", null, t);
    });
  }
...
function openEntryModal(mode, entry=null, forcedType=null){
  if(state.role==="viewer"){ alert("viewer は編集できません"); return; }
  const type = forcedType || entry?.type || state._moneyTab || "income";
  const cats = (type==="income") ? (state.master.incomeCategories||[]) :
               (type==="expense") ? (state.master.expenseCategories||[]) :
               (type==="transfer") ? (state.master.transferCategories||[]) :
               ["チャージ"]; 
...
if(type==="charge"){
  ...
  const ppSel = entry?.toAccountId || (state.prepaidCards?.[0]?.id ? prepaidAccountIdForCardId(state.prepaidCards[0].id) : "");
  const ppOpts = (state.prepaidCards||[])
    .map(p=>`<option value="${escapeHtml(prepaidAccountIdForCardId(p.id))}" ${prepaidAccountIdForCardId(p.id)===ppSel?"selected":""}>${escapeHtml(p.cardName||p.id)}</option>`).join("");

  return `
    <div>
      <div class="small">チャージ方法</div>
      <select id="m_chargeMethod" class="input">
        ${payMethods.map(p=>`<option ${p===paySel?"selected":""}>${escapeHtml(p)}</option>`).join("")}
      </select>
    </div>

    <div id="m_chargeCardWrap" style="display:none;">
      <div class="small">クレカ</div>
      <select id="m_chargeCard" class="input">${cardOpts}</select>
      <div class="small" style="margin-top:6px;">※クレカ利用分は「支払予定」に積み上げ</div>
    </div>

    <div>
      <div class="small">チャージ先（プリペイド）</div>
      <select id="m_toAccount" class="input">
        ${ppOpts || `<option value="">（プリペイド未登録）</option>`}
      </select>
    </div>
  `;
}
...
const syncPayUi = ()=>{
  ...
  if(type==="charge"){
    const pm = $("#m_chargeMethod") ? $("#m_chargeMethod").value : "";
    const isCard = (pm==="クレカ");
    if($("#m_chargeCardWrap")) $("#m_chargeCardWrap").style.display = isCard ? "" : "none";
    return;
  }
};
  if($("#m_payMethod")) $("#m_payMethod").addEventListener("change", syncPayUi);
  if($("#m_chargeMethod")) $("#m_chargeMethod").addEventListener("change", syncPayUi);
  syncPayUi();
...
$("#m_save").addEventListener("click", async ()=>{
    ...
    const paymentMethod = $("#m_payMethod") ? $("#m_payMethod").value : ( $("#m_chargeMethod") ? $("#m_chargeMethod").value : null );
    const creditCardId = $("#m_card") ? $("#m_card").value : ( $("#m_chargeCard") ? $("#m_chargeCard").value : null );
    const creditChannel = $("#m_cardChannel") ? $("#m_cardChannel").value : null;

// expense/charge: payment source resolution
let effectiveFrom = fromAccountId;
let effectiveCardId = null;

if(paymentMethod==="クレカ"){
  effectiveCardId = creditCardId || null;
  if(effectiveCardId){
    effectiveFrom = payableAccountIdForCardId(effectiveCardId);
  }else{
    effectiveFrom = null;
  }
}else if(type==="charge"){
  // charge: no "チャージ元" selection. infer from payment method
  const cashId = state.master?.cashAccountId || "cash";
  const banks = (state.master?.banks||[]).filter(x=>x.active!==false);
  const other = (state.master?.otherAccounts||[]).filter(x=>x.active!==false);
  const defaultBankId = state.master?.defaultBankAccountId || banks[0]?.id || other[0]?.id || null;
  if(paymentMethod==="現金") effectiveFrom = cashId;
  else effectiveFrom = defaultBankId;
}
